...
public partial class rptTenderRateComparision : System.Web.UI.Page
{ 
    #region Declaration
    ...
    private void LoadReport()
    {
      ...
      //  Int64 ToYearMonth = Convert.ToInt64(ddlToYearMonth.SelectedValue);
        ...
        {
            dt = Bus.rptTenderRateComparisionSelect(WcompanyId,ClientprojectId, FromYearMonth, WRegion); // IOWMapping Cost Detail radio button 
        }
        ...
        if ((dt != null && dt.Rows.Count > 0))
        {
          ...
          //  this.ReportViewer1.LocalReport.DisplayName = " Tender Cost Listing for- " + ddlProject.SelectedItem.Text + "_" + System.DateTime.UtcNow.AddMinutes(330).ToString("yyyyMMdd");
            this.ReportViewer1.LocalReport.DisplayName = " Tender Quote Comparision- " +  System.DateTime.UtcNow.AddMinutes(330).ToString("yyyyMMdd");
            try
            {
    ...
    private int IsValidSave()
    {
        int Error = 0;
         string DisplayError = "";
         if (ddlCompany.SelectedIndex == 0 || ddlFromYearMonth.SelectedIndex == 0 )
        {
         ...
         if (ddlFromYearMonth.SelectedIndex > 0 )
         {
             try
             {
