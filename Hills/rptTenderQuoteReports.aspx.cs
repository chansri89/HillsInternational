...
public partial class rptTenderQuoteReports : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            lblExceptionList.Text = "Project Tender Package Quote";
            LoadCompany();
        }
    }
    ...
    public void LoadReport(ReportMsg reportMsg)
    {
        ...
        string ReportTitle1 = ddlProject.SelectedItem.ToString();
        string RepPeriod = " From " ;
        string RunDate = "RunDate :" + System.DateTime.UtcNow.AddMinutes(330).ToString("dd-MM-yyyy HH:mm");
        string ErrorMsg = "..No Data Found for the Project..Pls Upload Tender..";
        DataTable dtAp = new DataTable();
        DataTable dt = new DataTable();
        dt = Bus.rptTenderListing(reportMsg); //
       // dt = Bus.rptTenderQuoteComparision(reportMsg); //rptTenderQuoteComparision
        if (dt != null && dt.Rows.Count > 0 )
        {
            ...
            else if (rbtType.SelectedValue == "1")
            {
                ...
                //IEnumerable<DataRow> dr = (from dt1 in dt.AsEnumerable() where (dt1.Field<string>("SrlNo") != "SrlNo" && 
                //                          ((dt1.Field<decimal>("SupplyAmount")) > 0 || (dt1.Field<decimal>("InstallAmount")) > 0))
                IEnumerable<DataRow> dr = (from dt1 in dt.AsEnumerable()
                                           where (dt1.Field<string>("SrlNo") != "SrlNo" &&    (dt1.Field<decimal>("ItemTotal")) > 0 )                   
                ...
                {
                    ErrorMsg = "..For Cost Details selection you need to upload Tender with Rates..";
                }
            }
            else
            {
        ...
        if ((dtAp != null && dtAp.Rows.Count > 0))
        {
            ...
            rep.ReportPath = reppath + "rptTenderReports.rdlc";
           // rep.ReportPath = reppath + "rptTenderComparision.rdlc";                
            //ReportTitle1 =" ";
            this.ReportViewer1.LocalReport.DisplayName = " Project- " + ReportTitle1+"_" + System.DateTime.UtcNow.AddMinutes(330).ToString("yyyyMMddHHmm");
            try
            {
