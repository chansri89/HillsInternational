...
public partial class NewExcelTenderPackageUploadWithIOWRev : System.Web.UI.Page
{
    #region Declaration
    #region XL
    ...
    //int DBError = 0;
    private static string UploadwithIOW = "IOW";
    #endregion
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            try
            {
                ...
                lblPgmHdr.Text = "Tender Package Upload -- Client BOQ with IOW Code -- Multi Sheet";
            }
            catch
            {
            ...
            ddlQuote.SelectedIndex = 1; // Hardcoded for Tender Package upload
    ...
    private void LoadClient()
    {
        ...
        var Cli = (from pm in PMList where pm.CompanyId == CompId select new { pm.ClientCode, pm.ClientName }).Distinct().ToList();
        if (Cli != null)
        {
    ...
    private int RevisionChanged()
    {
        ...
        try
        {
            ...
            {
                err = 1;
                ErrText = "New Revision Number Cannot be Less than Previous Revision Number..";
            }
            //else if (NRevno > oldRevNo)
            //{
            //    Panel1.Visible = true;
            //    err = 9;
            //}
    ...
    private void UploadInProgress()
    {
        ...
        {
            if (ddlCompany.SelectedIndex == 0 || ddlClient.SelectedIndex == 0 || ddlProject.SelectedIndex == 0)
            {
                ScriptManager.RegisterStartupScript(this, typeof(string), "Alert", "alert('" + "Company / Client/Project are Mandatory .. Ensure they are selected" + "');", true);
            }
            else
            {
    ...
    private List<ExcelSheetNameMsg> GetExcelShtNames(string excelFile)
    {
        ...
        try
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (WSheetName.Substring(0, 1) == "'") // SheetName is having space inbetween
                {
                    Windex = Windex + 1; // Include two single quotes in the length
                }
    ...
    private List<ExcelUploadMsg> MultiSheetDataUpload(List<ExcelSheetNameMsg> XLSht)
    {
        ...
        foreach (ExcelSheetNameMsg sht in XLSht)
        {
            ...
            try
            {
                ...
                //if (txtOnlyAmount.Text == "Y")
                //{
                //    var InstType = (from sup in excelUploadMsgList select IsNumeric(sup.Column5)).Max();
                //    if (InstType == true)
                //    {
                //        foreach (ExcelUploadMsg exp in excelUploadMsgList)
                //        {
                //            exp.Column46 = "1";
                //        }
                //    }
                ...
                //    }
                //}
                //var suppType = (from sup in excelUploadMsgList where Convert.ToInt32(Isnumeric(sup.Column5))
            ...
            if (linecount >= 0) // Upload as is else if header is specified then make >=1
            {
                ...
                #region TendPack
                        ...
                        //{
                            ...
                            //    //}
                            //    AmountType = "0"; // Excel Format fixed discussion 231011 with Mr. Janakiram/ Mr. Sudagar
                            //}
                ...
                #region Unwanted
                           // excelUpload.Column11 = (dr[9].ToString() == null || dr[9].ToString() == string.Empty) ? "" : dr[9].ToString(); //LSP_SP
                            ...
                            //excelUpload.Column12 = (dr[11].ToString() == null || dr[11].ToString() == string.Empty) ? "0" : dr[11].ToString(); //Auction Value
                ...
                {
                    excelUpload.Column45 = UploadwithIOW;
                    excelUpload.Column46 = "0"; //scs260415 used for only supply defaulted to 0
