...
public partial class NewExcelItemRateUploadRev : System.Web.UI.Page
{
    #region Declaration
    #region XL
        ...
        if (!Page.IsPostBack)
        {
           ...
           // ddlProject.Enabled = false;
            ...
            try
            {
                ...
                lblPgmHdr.Text = "Item Rate With Multi Sheet";
            }
            catch
            {
    ...
    private void UploadClick()
    {
        if (ddlCompany.SelectedIndex == 0 )
        {
            ScriptManager.RegisterStartupScript(this, typeof(string), "Alert", "alert('" + "Select Company and File to be uploaded" + "');", true);
        }
        else
        {
    ...
    private List<ExcelUploadMsg> MultiSheetDataUpload(List<ExcelSheetNameMsg> XLSht)
    {
        ...
        foreach (ExcelSheetNameMsg sht in XLSht)
        {
            ...
            {  //ConnectionString = "Provider=Microsoft.Jet.OleDb.4.0;Data Source=" + @XLfilepath + filename + ";Extended Properties='Excel 8.0;HDR=YES;IMEX=1'";
               ...
               // ConnectionString = "Provider=Microsoft.Jet.OleDb.4.0;Data Source=" + @XlFileName + ";Extended Properties='Excel 8.0;HDR=YES;IMEX=1'";
            }
            else
            {  //ConnectionString = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + @XLfilepath + @filename + ";Extended Properties='Excel 12.0;HDR=Yes;IMEX=1'"; //HDR =Yes measns data is having column heading
               // ConnectionString = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + @XLfilepath + @filename + ";Extended Properties='Excel 12.0;HDR=NO;IMEX=1'"; //HDR =Yes measns data is having column heading
                ...
                //ConnectionString = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + @XlFileName + ";Extended Properties='Excel 12.0;HDR=YES;IMEX=1'"; //HDR =Yes measns data is having column heading
            }
            ...
            try
            {
               ...
               // }
                excelUploadMsgList = ITRateUpload(excelData, TranType);
    ...
    private void CheckandUploadData(List<ExcelUploadMsg> excelUploadMsgList)
    {
              ...
              lblMessage.Text = lblMessage.Text + " chk xl data begin-"; //scs 251215
                 ChkResult = 0; // set to OK condition
                  ...
                  List<DepotSalesMsg> ChkData = CheckData(excelUploadMsgList, TranType);
                     ...
                     {
                         lblMessage.Text = " Item Rate Tempinsert begin-"; //scs 251215
        ...
        //{
            if (ChkResult == 0)
            {
                ...
                {
                    lblMessage.Text = "Inserting to Item Rate started";
                    ...
                    else
                    {
                        ...
                        {
                            ScriptManager.RegisterStartupScript(this, typeof(string), "Alert", "alert('" + " Failuer while inserting IT Rate Table...   " + Uploadinsert.Result.Substring(1, Uploadinsert.Result.Length - 1) + "');", true);
                ...
                {
                    ScriptManager.RegisterStartupScript(this, typeof(string), "Alert", "alert('" + " Failure while inserting Temp Table" + UpMsg.Result.Substring(1, UpMsg.Result.Length - 1) + "');", true);
    ...
    public List<ExcelUploadMsg> ITRateUpload(DataTable dtCoup,string TranType)
    {
        System.Globalization.DateTimeFormatInfo dateinfo = new System.Globalization.DateTimeFormatInfo();
       ...
       // dateinfo.ShortDatePattern = "dd-MM-yyyy";
        ...
        int linecount = 0; // if in excel connection string HDR is Yes the Hdr is taken care hence 1 else 0
        foreach (DataRow dr in dtCoup.Rows)
        {
           ...
           // string WDate = ""; string sDate = ""; //string WChar = "";
            ...
            if (linecount > 0) // Upload as is else if header is specified then make >=1
            {
                ...
                #region ITrate
                        if ((dr[0].ToString() != string.Empty && dr[2].ToString() != string.Empty && dr[3].ToString() != string.Empty && dr[4].ToString() != string.Empty)) //
                        {
                            excelUpload.Column1 = linecount.ToString(); // xl row number
                            excelUpload.Column2 = (dr[0].ToString() == null || dr[0].ToString() == string.Empty) ? string.Empty : dr[0].ToString(); // ItemCat
                            RemovedCharColumn = RemoveUnWantedCharacter(excelUpload.Column2);
                            excelUpload.Column2 = RemovedCharColumn;
                            excelUpload.Column3 = (dr[1].ToString() == null || dr[1].ToString() == string.Empty) ? string.Empty : dr[1].ToString(); // ItemSubCat
                            RemovedCharColumn = RemoveUnWantedCharacter(excelUpload.Column3);
                            excelUpload.Column3 = RemovedCharColumn;
                            excelUpload.Column4 = (dr[2].ToString() == null || dr[2].ToString() == string.Empty) ? string.Empty : dr[2].ToString(); //ItemGrp
                            RemovedCharColumn = RemoveUnWantedCharacter(excelUpload.Column4);
                            excelUpload.Column4 = RemovedCharColumn;
                            excelUpload.Column5 = (dr[3].ToString() == null || dr[3].ToString() == string.Empty) ? string.Empty : dr[3].ToString(); // ITCode
                            RemovedCharColumn = RemoveUnWantedCharacter(excelUpload.Column5);
                            excelUpload.Column5 = RemovedCharColumn;
                            excelUpload.Column6 = (dr[4].ToString() == null || dr[4].ToString() == string.Empty) ? string.Empty : dr[4].ToString(); // ITName
                            RemovedCharColumn = RemoveUnWantedCharacter(excelUpload.Column6);
                            excelUpload.Column6 = RemovedCharColumn;
                            excelUpload.Column7 = (dr[5].ToString() == null || dr[5].ToString() == string.Empty) ? string.Empty : dr[5].ToString(); //Item UOM
                            RemovedCharColumn = RemoveUnWantedCharacter(excelUpload.Column7);
                            excelUpload.Column7 = RemovedCharColumn;
                            excelUpload.Column8 = (dr[6].ToString() == null || dr[6].ToString() == string.Empty) ? string.Empty : dr[6].ToString(); // Make code
                            RemovedCharColumn = RemoveUnWantedCharacter(excelUpload.Column8);
                            excelUpload.Column8 = RemovedCharColumn;
                            excelUpload.Column9 = (dr[7].ToString() == null || dr[7].ToString() == string.Empty) ? string.Empty : dr[7].ToString(); //Makename
                            excelUpload.Column10 = (dr[8].ToString() == null || dr[8].ToString() == string.Empty) ? " " : dr[8].ToString(); //IsImpor
                            excelUpload.Column11 = (dr[9].ToString() == null || dr[9].ToString() == string.Empty) ? "" : dr[9].ToString(); //ITrate
                            excelUpload.Column12 = (dr[10].ToString() == null || dr[10].ToString() == string.Empty) ? "" : dr[10].ToString(); //Region
                            excelUpload.Column13 = (dr[11].ToString() == null || dr[11].ToString() == string.Empty) ? "" : dr[11].ToString(); //RateDAte
                            excelUpload.Column14 = (dr[12].ToString() == null || dr[12].ToString() == string.Empty) ? "" : dr[12].ToString(); //  Source
                            excelUpload.Column15 = (dr[13].ToString() == null || dr[13].ToString() == string.Empty) ? "" : dr[13].ToString(); // Disc
                            excelUpload.Column16 = (dr[14].ToString() == null || dr[14].ToString() == string.Empty) ? "0" : dr[14].ToString(); // gst
                             #region Unwanted
